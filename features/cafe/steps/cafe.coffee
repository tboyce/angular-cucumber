...
module.exports = ->
    ...
    @Then /should (see|not see) "([^"]*)" in the employee list/, (see, name)->
        if see == 'see'
            isEmployeeInList(name).should.be true, "expected #{name} to be in the employee list"
        else
            isEmployeeInList(name).should.be false, "expected #{name} not to be in the employee list"
